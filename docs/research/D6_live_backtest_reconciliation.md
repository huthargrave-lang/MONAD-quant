...
- **PART B — live side (the +37% → +1.5% exit-accounting gap).** It parses the 69 exported TQQQ paper fills, groups them by `exit_type`, and compounds three baskets: **CONFIRMED** = `{bracket_exit, stop_hit}` (the actually-filled exits — set matches `tools/ctx.py` exactly), **PROD** (the dashboard set), and **ALL**. It then builds an **inflation ladder**: start from CONFIRMED and add each artifact group in turn.
- **THE BRIDGE (Part C).** A qualitative reconciliation: the backtest headline was earned on morning-only (~3-bars/day) data with an adaptive-Kelly sizing the live bot doesn't use ([[F28]], fixed-10%), holdout-selected ([[F2]]), with optimistic fills. Strip the frequency artifact ([[F13]]/[[F14]], Part A) and the honest full-session hourly edge ≈ 0 — which rounds to the same flat as the live CONFIRMED +1.55%.
...
The +37% headline is manufactured by 15 non-confirmed exits, and the WR is lifted from 45.1% to 56.5% by the same rows. The two artifact mechanisms are **distinct** (a correction folded from the panel — the study had lumped them):

- **`target_hit` (6 rows) is a genuine MARKED-price artifact, code-proven.** `live/trader.py::_infer_bracket_exit` returns the **TP price** whenever IBKR fill data is unavailable/ambiguous (lines 84/101-102). All 6 `target_hit` returns are exactly **~+1.00%** (the TP level) — never a confirmed fill.
...
Either way, both groups sit outside CONFIRMED, so the **honest realized edge is +1.55% (flat)** regardless of how the time_exit anomaly is ultimately explained.

### THE BRIDGE — the two honest numbers agree (qualitatively)

The backtest Sharpe-25 headline was earned on **morning-only (~3-bars/day) data** ([[F13]]/[[F14]]) with an **adaptive-Kelly sizing the live bot doesn't use** (fixed-10%, [[F28]]), **holdout-selected** ([[F2]]), and with optimistic fills. Strip the dominant frequency artifact (Part A) and the honest **full-session hourly** edge ≈ 0 / negative-Sharpe sleeve. The live CONFIRMED edge is **+1.55%** over ~3 months at WR 45%. **Both round to flat.**
...
**The live bot is flat because it trades a coarse-timescale (multi-day mean-reversion) signal at an HOURLY frequency where that edge does not exist.** Re-derived on the live instrument (Part A): the lag-1 return autocorrelation that fuels the signal is negative at the daily timescale (QQQ −0.080, TQQQ −0.055) and the dip-buy sleeve nets **+16–17 bps/trade**; at the 7-bars/day frequency the bot actually trades, the autocorrelation is ≈0 and the sleeve nets **−3 bps (negative Sharpe)**. The backtest's **Sharpe-25 headline was a morning-only (~3-bars/day) sampling artifact** ([[F13]]/[[F14]]), compounded by an unused adaptive-Kelly sizing ([[F28]]), holdout selection ([[F2]]), and optimistic fills. The live dashboard's **+37% is a SEPARATE exit-accounting artifact** (Part B): only 51 of 69 fills are actually-filled exits, netting **+1.55%** flat; the other 18 — 6 TP-marked `target_hit` (~+1.00% each, code-proven non-fills) plus 9 anomalous all-positive, all-at-10-bar-cap `time_exit` — manufacture the +37%. **The two HONEST numbers — full-session hourly backtest ≈ flat and live CONFIRMED +1.55% — agree.** Nothing is "broken" in the bot; it faithfully trades a signal that has no edge at its frequency. F28's "structurally disconnected" is quantified: the disconnect is **dominated by BAR-FREQUENCY** (coarse-timescale mean-reversion sampled as hourly noise), with sizing / selection / cost as secondary multipliers.
...
- **(minor, corrected) `time_exit` ≠ "marked price."** `target_hit` is a code-proven TP-marked non-fill (all ~+1.00%); `time_exit` gets a real `cancel_and_close` fill but has an anomalous all-positive / all-at-10-bar-cap distribution. The two mechanisms are separated; the CONFIRMED=+1.55% conclusion is unchanged.
...
- **CONFIRMED=+1.55% assumes target_hit/time_exit are not genuine edge.** target_hit is code-proven marked; time_exit's exclusion rests on its anomalous distribution, not a "never filled" mechanism, and was not cross-checked against the raw IBKR execution log. If some time_exit rows are real edge, the honest number sits modestly above +1.55%.
...
- **Small live sample** (~3mo, 51 CONFIRMED trades, WR 45%): +1.55% is small-positive/flat, consistent with "no edge" but not a powered zero on the live data alone — the no-edge claim leans on the Part A frequency argument and the broader [[D6]] arc.
