...
**3. Forward-looking, the product clears the income goal but not the drawdown goal.**
At 2026 starting yields the 60/40 should return ~5–6%/yr — comfortably above the ~3.75% APY target ([[D4]]) — but with equity-like tail risk (~−23% median worst drawdown). The original "near-zero drawdown" aspiration is **unattainable by any honest static or active build** in this program (#8).
...
- **Idea graph:** `venv/bin/python tools/ctx.py web <E25..E32 | F34..F41 | D6 | D4>` · `ctx why <node>` · `ctx neighbors <node>`
