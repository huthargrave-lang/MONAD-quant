...
- **Idea graph:** `venv/bin/python tools/ctx.py web <E25..E39 | F34..F49 | D6 | D4>` · `ctx why <node>` · `ctx neighbors <node>`
- **Reproduce any study:** `venv/bin/python tools/<name>_study.py` (deterministic, seed=0; studies #6/#8/#11–#15 fetch once to `/tmp`).
