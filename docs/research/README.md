...
Studies 1–10 are the **active-vs-static** arc (does MONAD's engine beat a static allocation, and what is the honest product). Study 11 opens a follow-on **product-universe** program (is there a *better asset universe* than the static 60/40 the arc landed on).
...
- **Idea graph:** `venv/bin/python tools/ctx.py web <E25..E35 | F34..F44 | D6 | D4>` · `ctx why <node>` · `ctx neighbors <node>`
- **Reproduce any study:** `venv/bin/python tools/<name>_study.py` (deterministic, seed=0; studies #6/#8/#11 fetch once to `/tmp`).
