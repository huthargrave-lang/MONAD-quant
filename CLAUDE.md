...
## 13–22. Optimization history, per-mode results, audits & roadmaps — ARCHIVED

These dated sections (BTC/QQQ/TQQQ/GDXU/SOXL/LABU/TNA optimization logs, the sweep-tool
usage notes, pending-close architecture, the 2026-04 project reviews, and the future-work
roadmaps) were moved to **[`docs/history/MODEL_HISTORY.md`](docs/history/MODEL_HISTORY.md)**
to keep this auto-loaded file lean. They are **dated history with superseded performance
numbers** — for current truth run `ctx web --live` / `ctx perf`; for the live roadmap see
`IMPROVEMENT_PLAN.md`; for ops see `OPERATIONS.md`.
