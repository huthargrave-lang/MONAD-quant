...
### F266 — Post-gap bracket inference booked a stop fill as a target hit
IBKR fill data is current-day scoped, so `_infer_bracket_exit` only runs when the bracket filled on a PREVIOUS day — exactly when an overnight gap has severed the current price from the fill. The old rule (ref_price >= tp -> target_hit, else a nearest-level tiebreak) therefore read the one input the situation destroys. Live on 2026-07-31: TQQQ closed 63.44, gapped to a 65.74 open, and a 63.46 stop fill was booked target_hit @ 64.42 (+1.0034%). The 07-30 15:32-16:00 tape shows low 63.26 (through the stop) and high 63.63 (target never within 79c). A -0.50% loss entered the book as a +1.00% win. Not merely one trade's PnL: exit_type and return_pct feed the rolling win rate and adaptive Kelly sizes from it, so a false win raises the next positions. Fixed by first-touch along the bar path since entry, same-bar ambiguity resolving to the stop to match the backtest fill model (engine.py:247); when the path cannot answer it records estimated_close rather than naming a leg. Trade 105 corrected in state.db (53->52 wins, 50.5%->49.5% WR, +29.94%->+28.44%). Same overnight-gap family as [[F47]]. Does NOT resolve [[H28]], which is about provenance — an inferred stop_hit is still byte-identical to a confirmed one — not correctness.
Links: [[F47|relates]] · [[H28|relates]].
_— captured development@c5f7e7c, 2026-08-02_

### F267 — Unmodelled commission is 24% of the live account loss — measured from real fills
The 2026-07-25 handoff listed 'No cost model reaches main.py' as an owner's call, deferred because wiring costs in invalidates every stored sweep result. Nobody had measured the magnitude, so the call could not be made on evidence. Measured now from 339 real IBKR CommissionReport lines in local_logs/trader.log: median $1.1474 per full-clip fill, so $2.2948 round-trip against a median recorded notional of $10,157 = 0.0226% of the position per trade. Over the 82 CONFIRMED fills that is 1.853% notional / 0.185% account; the confirmed account result is -0.768% (see [[F160]] for why account and notional must not be conflated). So commission alone explains 24% of the observed loss, and main.py models none of it. Relative to the live bracket it is 2.3% of a +1.00% winning move and 4.5% of a -0.50% losing one — too small to flip any single trade's sign (win rate is 49.5% gross and net), but a systematic one-way drag that every backtest headline omits. Two corrections to the handoff's own text: (a) 'sweep_costs.py is imported only by sweep.py' is now STALE — it also reaches tools/strategy_funnel.py and tools/walkforward_eval.py, three non-test consumers; (b) the claim that commission is modelled nowhere HOLDS — zero references in src/, main.py or live/. Does not itself decide the owner's call, but prices it: the cost of wiring is invalidated sweep caches, and the cost of not wiring is a known ~0.023%/round-trip overstatement. Refines [[F242]], which established the cost model is a single number and the strategy trades where it is wrong.
Links: [[F242|refines]] · [[F160|relates]] · [[F157|relates]].
_— captured development@3329b3c, 2026-08-02_

### F268 — E19's dead-end ledger is 3/4 unreproducible — the mechanism it measured was deleted
E19 is the consolidated 'tried & abandoned' ledger, carried as 9 figures with 0 reachable docs, and it is cited to stop people re-trying those levers. Checked whether the figures can be reconstructed. Answer per figure, and the blocker is usually NOT missing data — it is that the code the figure measured no longer exists. (1) 'strict 50-MA gate filtered 71/83 trades': STRONG_BULL_REQUIRE_50MA has ZERO readers in src/ or main.py, and the 6-state regime gate it operated inside was stripped from generate_trades ([[F26]]) — the function still takes use_slope_regime/longs_only but its body applies only the vol_regime/trend_direction filter. NOT reproducible. (2) '5% STRONG_BULL target dropped WR 49.4%->33.7%': TARGET_GAIN_PCT_STRONG_BULL also has ZERO readers — same dead-knob shape as [[F145]]. NOT reproducible. (3) 'RSI 38-42 extra entries dropped WR 68.8%->57.9%': depended on regime-gated STRONG_BULL entries; same stripped gate. NOT reproducible. (4) 'opposing-signal exit hurt TQQQ': this one is ALIVE — USE_OPPOSING_SIGNAL_EXIT is read at src/backtest/runner.py:143-145 and set by sweep.py:274 — and data/cache/TQQQ_1h.csv exists, so it is reproducible ON THIS MACHINE. But the repo tracks 0 bar CSVs, so it is not reproducible from a clean clone or in CI. The wider consequence is that E19's authority is narrower than its use: three of its four figures measured levers that no longer run, so they cannot bear on a decision to re-add those levers today — anyone re-introducing a STRONG_BULL target would be building on a gate that does not execute, and E19's numbers would not describe the result. Recording non-recovery as the result, per the backlog's own instruction.
Links: [[E19|refines]] · [[F26|relates]] · [[F145|relates]].
_— captured development@aa2882a, 2026-08-02_

### F269 — H21 tested and retired: the 0.05 threshold is 8 trades on one panel, in the wrong units
Tested H21 (raise STRONG_BULL_SOFT_50MA_PCT from 0.02 to 0.05) on the real TQQQ 1h panel, 3425 bars 2024-07-01..2026-06-18, through the production build_features -> generate_trades -> compute_trade_returns path. The knob is genuinely live (engine.py:161-180), unlike the dead STRONG_BULL levers in [[F268]]. Measured: gate OFF admits 433 candidates; at 0.02 it blocks 21 (4.8%); at 0.05 it blocks 13 (3.0%). So H21 LOOSENS the gate and the entire proposal moves 8 trades in two years. Those 8 are 6 targets (+1.000%) and 2 stops (-0.500%), summing +0.0500 — which is the whole +14.7% book improvement (412 trades sum +0.3400 vs 420 trades sum +0.3900). Significance, computed correctly: sd of one trade 0.0073, so sd of a sum of 8 is 0.0207 and the effect is 2.42 sigma; a bootstrap over 20000 random 8-trade draws from the same book beats it 3.7% of the time. Nominally p<0.05 on a single in-sample panel with n=8 — and [[F18]] established this project's t-stats run about 3x overstated, which puts the honest figure under 1 sigma. Commission ([[F267]]) removes 0.0018 of the 0.0500, immaterial here. RETIRE AS STATED, for a structural reason that outranks the statistics: on hourly, ma_50d is a 50-HOUR mean (~2.5 trading days), while H21's 5% came from DAILY observations of distance below a 50-DAY MA. The proposal imports a number across timeframes into units where it does not mean the same thing, so 0.05 is not 'the 5% gate' H21 describes. If the direction is worth pursuing, re-derive the threshold in the hourly MA's own units rather than porting the daily one. Correction to my own working: I first reported this as 6.84 sigma by dividing the summed delta by a SINGLE-trade sd instead of sd*sqrt(8) — the corrected figure is 2.42.
Links: [[H21|relates]] · [[F18|relates]] · [[F268|relates]] · [[F267|relates]].
_— captured development@ef751f2, 2026-08-02_

### F270 — H34's reporting mismatch is real but 93% aggregation, and it sits inside an 11x unit error
Tested H34 (dashboard vs alert PnL mismatch) on the live 105-trade book. CONFIRMED, and quantified. The alert path (live/state.py::get_trade_summary) sums raw returns over ALL trades: +28.4390%. The dashboard (live/dashboard.py) compounds prod(1+r)-1 over PROD-filtered trades: +31.3746%. Mismatch 2.9357pp, 9.4% of the dashboard figure. Decomposed, H34's two stated causes are NOT equal partners: the population difference (all 105 -> 102 prod, excluding estimated_close and paper_reset) moves it -0.2362pp, while the aggregation choice (simple sum -> compounded) moves it +3.1719pp. Aggregation is ~93% of the gap; picking one trade population barely matters, picking one aggregation is the whole fix. H34's proposed src/analysis/performance.py does not exist, so the two paths are still independent implementations. BUT the larger result is that fixing H34 alone would make the two paths agree on a number that is still wrong by an order of magnitude. Each trade deploys 10% of equity ([[F160]]), so compounding RAW position returns treats every trade as if 100% of equity were deployed. Compounding the same PROD series in account units gives +2.8509% against the dashboard's +31.3746% — an 11.0x overstatement, and the compounding step AMPLIFIES it rather than merely relabelling it. So the honest ordering is: F160 first (report account units), H34 second (pick one aggregation), and the population question last. Reconciling two views of an 11x-overstated figure would buy consistency and no accuracy. Does not retire H34 — it is real — but re-scopes it from 'three co-equal fixes' to 'one aggregation choice, downstream of a unit error that dominates it'.
Links: [[H34|refines]] · [[F160|builds_on]].
_— captured development@6db193b, 2026-08-02_

### F271 — H36 retired: its caching targets are 2% of the cost and the hook path never builds the graph
Re-measured H36 (ctx computed-graph cache) on the current repo at 494 web nodes, confirming and updating the profile in tests/test_h36_graph_cost_profile.py which was taken at 415. That analysis existed only in a test docstring, never as a web node, which is why the backlog kept surfacing H36 as untested — recording it here closes that. Measured, best of 3: _manifest 0.4ms, _parse_web 29.1ms, build_graph(include_code=False) 35.2ms, build_graph(include_code=True) 1491.1ms. The epistemic layer H36 names as its caching target is 2.0% of a full graph build; the code-side AST walk is the other 98%. Cost has grown 28% since the earlier profile (1161.3 -> 1491.1ms). The decisive operational fact is new: cmd_brief makes ZERO build_graph calls, so `ctx brief` — the only path that runs automatically, via ops/session_orient.py on every SessionStart — costs 0.13s against a 20s hook budget, roughly 150x headroom. The graph cost does not touch the automatic path at all; `ctx health` at 1.70s is interactive. So there is no operational pressure behind the proposal. RETIRE H36 AS STATED, on three grounds: its two named targets are worth 2% between them, its own '~0.5s for a full graph build' figure is now 3x stale in the direction of understating, and its safety constraint ('never serve stale epistemic data') inverts the proposal — the research web is exactly the layer that must never be cached, because a guard passing against data that no longer exists is this project's core failure mode, while the code AST is mtime-invalidable and is where the cost actually is. If revisited, cache the code AST only. Worth re-checking when build_graph(include_code=True) approaches a few seconds; at 28%/profile-interval growth that is not far off, but it will bite interactive use, not the hook.
Links: [[H36|relates]].
_— captured development@5d0fd58, 2026-08-02_

### F272 — H37 audited: the classifier blast radius is 29 links, and today all of them are right
H37 asked for three things about _classify_edge (the nearest-verb heuristic that types untyped [[ID]] links): dump every untyped link with its inferred type, quantify false-pos/neg on the stale-cite boundary, and consider requiring explicit types into non-current nodes. Done on the live 495-node web. Counts: 1822 links, 1510 typed and 312 untyped (17.1%). Inferred types on the untyped: relates 223, drives 23, builds_on 21, resolves 11, produces 10, evidenced_by 9, refines 8, supports 5, supersedes 2 — so 34 of 312 (10.9%) land on a RELIANCE type. The important number is smaller than any of those. A misclassification only changes behaviour where it crosses the stale-cite boundary, i.e. on links INTO a non-current node, and there are 11 such nodes attracting 47 links: 18 already typed explicitly and 29 untyped. So the blast radius of a classifier error is 29 links, 9.3% of the untyped population, not 312. Of those 29 exactly ONE infers a reliance type: a supports edge into F4. Hand-checked, and it is correct — the prose reads Supports immediately before the link. Better still, the citing node is F8, which is itself superseded by F13, so a superseded node cites a superseded node and the lint reports 0 problems and 0 advisories. Measured false positives on the stale-cite boundary today: zero. H37 is a real risk that has not yet fired. On its third ask: requiring explicit types for links into non-current nodes is now cheap and worth doing, because the population is 29 annotations, 18 of the 47 are already explicit, and it removes the heuristic from the only place it can alter lint — leaving prose-cue inference to the 265 links where being wrong costs nothing. Retire the audit half of H37; keep the explicit-type proposal as a bounded piece of work with a known size.
Links: [[H37|relates]] · [[F27|relates]].
_— captured development@5cf8326, 2026-08-03_

### F273 — H4 conflates two claims: the holdout NUMBER is inflated, but the holdout SELECTOR is informative
