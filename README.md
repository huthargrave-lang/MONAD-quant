...
> Long-only mean-reversion engine for leveraged ETFs and BTC, plus a **Context Kit** —
> a queryable research + code knowledge graph with an interactive 3D map.
> Currently in **paper-testing / validation**; the active hourly edge did not survive
> honest, full-session backtests (see [Research](#research--honest-edge) below).
