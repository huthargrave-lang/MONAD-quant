...
.prompt{min-width:0;padding:7px 8px;border:.5px solid var(--bd);border-radius:8px;background:var(--surf);color:var(--mut)}
.prompt-top{display:flex;align-items:center;justify-content:space-between;gap:6px;margin-bottom:4px;color:var(--tx);font-size:12px;font-weight:600}
.copy{flex:0 0 auto;height:22px;padding:0 7px;border-radius:6px;background:var(--bg);font-size:11px}
.prompt p{margin:0;line-height:1.35;font-size:12px;white-space:pre-wrap;overflow-wrap:anywhere}
.cmd{font-family:ui-monospace,SFMono-Regular,Menlo,monospace;color:var(--tx)}
...
<div class="bar"><span id="pills"></span><input id="q" placeholder="search…" style="margin-left:auto;width:170px"><span id="match"></span><button id="flat">flat</button><button id="fit">fit</button><button id="reset">reset</button><a id="back" href="/screener/draft" title="Back to the screener">&larr; dashboard</a></div>
<div class="legend" id="legend"></div>
<div id="offline" style="display:none;padding:14px 16px;margin:12px 16px;border-left:3px solid var(--warning);border-radius:0 8px 8px 0;background:var(--surface);color:var(--ink-2);font-size:13px;line-height:1.5"><b>The graph could not be drawn.</b> The layout library is bundled into this page, so this is not a network problem — d3 failed to evaluate. The node data IS present here; only the drawing did not happen.</div>
...
function labelText(n){return idea.has(n.k)?n.id.replace('area:',''):'';}
...
/* module + config nodes are 33% of the graph and 19% of the edges, and they are   scaffolding rather than research content — default them OFF so the map opens on the   idea web. Both pills toggle them straight back on. */const on={};Object.keys(LBL).forEach(k=>on[k]=true);on._sup=true;on.module=false;on.config=false;let visNodes=nodes,visLinks=links;function recomputeVisible(){visNodes=nodes.filter(vis);visLinks=links.filter(l=>vis(l.source)&&vis(l.target));for(let i=0;i<visNodes.length;i++){const n=visNodes[i];if(!n.p)n.p=projectNode(n);}/* paint() rewrites every element's display right after this, so the culling cache   must forget what it last wrote or a stale flag would suppress the re-hide. */for(let i=0;i<visNodes.length;i++){const ne=nodeEls&&nodeEls[visNodes[i].i];if(ne)ne.__culled=false;}for(let li=0;li<visLinks.length;li++){const le=linkEls&&linkEls[visLinks[li]._i];if(le)le.__culled=false;}}
...
function screenXY(d,lx=0,ly=0){const p=d.p||projectNode(d),k=zoomK();return {x:(p.x+lx*p.s)*k+(zt.x||0),y:(p.y+ly*p.s)*k+(zt.y||0)};}
function visualRadius(d){const hot=sel===d.i||hit(d),near=sel!==null&&adj[sel].has(d.i);return rad(d)*(hot?3.15:(near?2.8:2.25))+8;}
function labelSide(d){const c=screenXY(d,0,0).x;return W-c>=c?1:-1;}
function labelOffset(d){return visualRadius(d)+(sel===d.i||hit(d)?17:9);}
function labelY(d){return sel===d.i||hit(d)?0:2;}
function labelBox(d){const txt=labelText(d);if(!txt||!vis(d))return null;const side=labelSide(d),lx=side*labelOffset(d),ly=labelY(d),p=d.p||projectNode(d),k=zoomK(),s=screenXY(d,lx,ly),fs=labelFont(d),w=Math.max(14,txt.length*fs*.64*p.s*k),h=Math.max(12,fs*1.45*p.s*k);return {left:side<0?s.x-w:s.x,right:side<0?s.x:s.x+w,top:s.y-h*.55,bottom:s.y+h*.55,node:d};}
function boxesOverlap(a,b,pad=4){return a&&b&&a.left-pad<b.right&&a.right+pad>b.left&&a.top-pad<b.bottom&&a.bottom+pad>b.top;}
...
function layoutLabels(){node.select('.node-label').attr('x',d=>labelSide(d)*labelOffset(d)).attr('y',labelY).attr('text-anchor',d=>labelSide(d)<0?'end':'start').attr('font-size',d=>labelFont(d)+'px');const keep=[],shown=new Set();/* SVG <text> is the measured dominant frame cost in motion (glyphs re-rasterise   under every transform change): hiding all text lifted the cruise orbit 6.2 -> 25   fps where stripping every blur filter managed 5.1 -> 6.2. So while the view is   MOVING (fastRender), only the selection, search hits and the 12 highest-priority   labels paint; the full collision-laid set returns the moment the view settles. */let cand=nodes.filter(n=>labelText(n)&&vis(n)).sort((a,b)=>labelPriority(b)-labelPriority(a));if(fastRender){const forced=cand.filter(n=>n.i===sel||hit(n));cand=forced.concat(cand.filter(n=>n.i!==sel&&!hit(n)).slice(0,12));}cand.forEach(n=>{const b=labelBox(n);if(!b)return;const inFrame=b.right>-8&&b.left<W+8&&b.bottom>-8&&b.top<H+8,forced=n.i===sel||hit(n),collide=keep.some(k=>boxesOverlap(b,k,forced?10:6));if(forced||(inFrame&&!collide)){shown.add(n.i);keep.push(b);}});/* Hidden labels must be display:none, NOT opacity 0: Chromium still pays for   zero-opacity SVG <text> on every frame — measured during a cruise with only 4   labels at opacity>0, flipping the rest from opacity-0 to display:none took the   orbit from ~11 to ~28 fps. Opacity is kept solely for the dimmed 0.26 state. */node.select('.node-label').style('display',d=>shown.has(d.i)?null:'none').attr('opacity',d=>shown.has(d.i)?((sel!==null&&d.i!==sel&&!adj[sel].has(d.i))?0.26:1):0);}
...
function filterGlow(d){return isHot(d)?'url(#orbGlowStrong)':(fastRender&&!isNear(d)?null:'url(#orbGlow)');}
...
function setFastRender(on){if(fastTimer){clearTimeout(fastTimer);fastTimer=null;}if(fastRender===on)return;fastRender=on;svgEl.classList.toggle('fast-render',on);svgEl.dataset.fastRender=on?'1':'0';paint();layoutLabels();}
...
function transformForBounds(vs,pad=70,maxScale=2.2){const ps=vs.map(projectNode).filter(p=>Number.isFinite(p.x)&&Number.isFinite(p.y));if(!ps.length)return null;const xs=ps.map(p=>p.x),ys=ps.map(p=>p.y),minX=Math.min(...xs),maxX=Math.max(...xs),minY=Math.min(...ys),maxY=Math.max(...ys);const dx=Math.max(maxX-minX,1),dy=Math.max(maxY-minY,1),scale=Math.max(.25,Math.min(maxScale,.92*Math.min(W/(dx+pad),H/(dy+pad))));return d3.zoomIdentity.translate(W/2-scale*(minX+dx/2),H/2-scale*(minY+dy/2)).scale(scale);}
function transformForEgo(d,pad=115,maxScale=2.05){const ns=egoNodes(d),c=projectNode(d);if(!ns.length||!Number.isFinite(c.x)||!Number.isFinite(c.y))return transformForBounds(ns,pad,maxScale);const ps=ns.map(projectNode).filter(p=>Number.isFinite(p.x)&&Number.isFinite(p.y));const dx=Math.max(1,...ps.map(p=>Math.abs(p.x-c.x))),dy=Math.max(1,...ps.map(p=>Math.abs(p.y-c.y))),scale=Math.max(.3,Math.min(maxScale,.88*Math.min(W/(dx*2+pad),H/(dy*2+pad))));return d3.zoomIdentity.translate(W/2-c.x*scale,H/2-c.y*scale).scale(scale);}
function applyTransform(t,skipLabel=false){if(!t)return;svg.interrupt();suppressZoomLabel=skipLabel;svg.call(zoom.transform,t);suppressZoomLabel=false;}
function transitionTransform(t,dur=500){if(!t)return;setFastRender(true);svg.transition().duration(dur).call(zoom.transform,t).on('end',()=>settleRenderQuality()).on('interrupt',()=>settleRenderQuality(80));}
function focusNode(d,scale=1.7){const p=projectNode(d);if(!Number.isFinite(p.x)||!Number.isFinite(p.y))return;transitionTransform(d3.zoomIdentity.translate(W/2-p.x*scale,H/2-p.y*scale).scale(scale),420);}
function fitVisible(){transitionTransform(transformForBounds(nodes.filter(visibleFinite),70,2.2),500);}
function applyEgoFrame(d,pad=115,maxScale=2.05,skipLabel=false){applyTransform(transformForEgo(d,pad,maxScale),skipLabel);}
function normAngle(a){const tau=Math.PI*2;return ((a+Math.PI)%tau+tau)%tau-Math.PI;}
function setOrbit(rx,ry,draw=true){orbit.rx=Math.max(-1.05,Math.min(1.05,rx));orbit.ry=normAngle(ry);if(draw)render();}
...
function moveOrbit(ev){if(!orbitDrag)return;ev.preventDefault();const dx=ev.clientX-orbitDrag.x,dy=ev.clientY-orbitDrag.y;if(Math.abs(dx)+Math.abs(dy)>2)orbitDrag.moved=true;setOrbit(orbitDrag.rx+dy*.006,orbitDrag.ry+dx*.006);}
function endOrbit(ev){if(!orbitDrag)return;if(orbitDrag.moved)blockClick=true;try{svgEl.releasePointerCapture(orbitDrag.pid);}catch(e){}orbitDrag=null;svgEl.classList.remove('orbiting');settleRenderQuality();}
function rawTarget(d){return d.id.replace(/^(code|cfg|mod):/,'').replace(/^area:/,'');}
function impactTarget(d){const t=rawTarget(d);if(d.k==='config')return t.startsWith('config.')?t:'config.'+t;if(t.includes('::'))return t.split('::').pop();return t;}
function nodeRef(d){return d.id+' ('+(LBL[d.k]||d.k)+') — '+d.title;}
function neighborSummary(nb){const out=[];Object.keys(nb).sort().forEach(k=>nb[k].forEach(([i,a])=>out.push(a+nodes[i].id+' via '+k)));return out.length?out.slice(0,10).join(', '):'no immediate graph neighbors';}
function makePrompts(d,nb){const ref=nodeRef(d),near=neighborSummary(nb),target=rawTarget(d),impact=impactTarget(d),ps=[];
 if(idea.has(d.k)&&d.k!=='area'){
  ps.push({t:'Orient',p:'Use `ctx web '+d.id+'`, `ctx neighbors '+d.id+'`, and `ctx why '+d.id+'` to explain this selected context-map node.\n\nNode: '+ref+'\nNeighbors: '+near+'\n\nSummarize what it means, why it matters, and what assumptions I should check before acting on it.'});
  ps.push({t:'Evidence Chain',p:'Trace the evidence and provenance for '+d.id+'. Use `ctx why '+d.id+'` first, then follow any important neighbor nodes.\n\nCall out strong evidence, weak evidence, superseded claims, and the next source I should inspect.'});
  ps.push({t:'Contradictions',p:'Use `ctx contradicts '+d.id+'` and the graph neighbors to find what this node overturns or is overturned by.\n\nExplain whether '+d.id+' still looks current, partially stale, or risky to rely on.'});
  ps.push({t:'Next Move',p:'Given '+ref+', propose the smallest useful next experiment, code inspection, or note update.\n\nInclude exact `ctx` commands and any files that should be read before making changes.'});
 }else if(d.k==='area'){
  ps.push({t:'Area Brief',p:'Use `ctx brief '+target+' --task "explore this area"` and `ctx map '+target+'` to orient me to this code area.\n\nExplain the major files, active findings, and the safest first file to inspect.'});
  ps.push({t:'Coverage',p:'Use `ctx tests '+target+'` plus the visible graph neighbors to assess test coverage for area:'+target+'.\n\nIdentify missing coverage, stale assumptions, and one high-value test to add.'});
  ps.push({t:'Frontier',p:'Use `ctx frontier "'+target+' UI/context-map work"` and this node neighborhood: '+near+'.\n\nRecommend the next 3 context nodes to inspect and why.'});
 }else if(d.k==='module'){
  ps.push({t:'Module Impact',p:'Use `ctx impact '+impact+'` and `ctx defs '+target+'` to inspect this module.\n\nSummarize callers, tests, protected-boundary risk, and the safest edit surface.'});
  ps.push({t:'Research Links',p:'Starting from '+ref+', follow graph neighbors: '+near+'.\n\nIdentify which findings or decisions this module supports and what could be invalidated by edits.'});
  ps.push({t:'Test Plan',p:'For module '+target+', derive a focused verification plan.\n\nUse `ctx covers '+impact+'` if a symbol is available, otherwise inspect importer/test neighbors and propose the smallest useful unittest command.'});
 }else if(d.k==='config'){
  ps.push({t:'Config Value',p:'Use `ctx config '+target+'` and `ctx impact config.'+target+'` to explain this config node.\n\nSummarize current value, consumers, protected-boundary risk, and why changing it is or is not safe.'});
  ps.push({t:'Consumers',p:'Trace every visible consumer of cfg:'+target+'.\n\nGraph neighbors: '+near+'\n\nGroup consumers by live path, analysis path, tests, and docs.'});
  ps.push({t:'Validation',p:'Design a validation checklist for changing config.'+target+'.\n\nInclude guard commands, realistic backtest/perf checks if relevant, and any research-web notes to update.'});
 }else{
  const isFile=impact.endsWith('.py')||impact.includes('/');
  const inspect=isFile?'`ctx impact '+impact+'` and `ctx defs '+impact+'`':'`ctx impact '+impact+'`, `ctx where '+impact+'`, and `ctx covers '+impact+'`';
  const trace=isFile?'Use `ctx impact '+impact+'`, `ctx defs '+impact+'`, and graph neighbors':'Use `ctx usages '+impact+'` and graph neighbors';
  ps.push({t:'Code Impact',p:'Use '+inspect+' to inspect this code node.\n\nNode: '+ref+'\nNeighbors: '+near+'\n\nExplain blast radius, tests, and whether edits touch protected live paths.'});
  ps.push({t:'Usage Trace',p:trace+' to trace how '+impact+' is used.\n\nSeparate direct callers, config dependencies, tests, and research claims tied to this '+(isFile?'file.':'symbol.')});
  ps.push({t:'Safe Edit',p:'Before editing '+target+', build a safe edit plan.\n\nList files to read, `ctx can_edit` checks, focused tests, and any context-web finding that should be captured afterward.'});
 }
 return ps;}
...
/* The initial force-layout settle used to paint EVERY tick: ~70 ticks x a full-graph
   software-rendered frame meant the page spent its whole first impression janking.
   The simulation math still runs every tick (the layout is unchanged); only the paint
   is decimated 3:1 while settling. Measured (Chromium, software-rendered container,
   564 visible nodes): load->settled 18.5s -> 8.5s from this change alone, ~7s once
   the label display:none fix (see layoutLabels) landed on top. */
let settleTick=0;
sim.on('tick',()=>{relax3D(sim.alpha());if(!simSettling||settleTick++%3===0)render();});
let userFramed=false;
sim.on('end',()=>{simSettling=false;render();settleRenderQuality(140);
  /* Only the FIRST settle, and only if the reader has not already chosen a view: an
     auto-fit that overrode a deliberate zoom or a selected node would be the map
     yanking the camera out of the reader's hands. */
  if(!userFramed&&sel===null){userFramed=true;fitVisible();}});
...
def _render_graph_html(G, adj):
    ...
    return (_GRAPH_HTML.replace("__PROJECT__", proj)
            ...
            .replace("__D3__", _d3_source())
            .replace("__DATA__", comp))


def _d3_source():
    """d3, inlined from the copy vendored in this repo.

    It used to load from cdnjs at runtime, which made the map's correctness depend on a
    third party being reachable at the moment someone opened it: a failed or slow fetch
    rendered the page with "0 nodes" and no graph, on a page whose data was fully
    present. Every other surface here is self-contained; this one now is too."""
    path = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                        "vendor", "d3.v7.8.5.min.js")
    try:
        with open(path, encoding="utf-8") as fh:
            return fh.read()
    except OSError:
        # Better a map that says why it is blank than one that silently draws nothing.
        return ("window.__D3_MISSING__=1;console.error("
                "'d3 is not vendored at tools/vendor/d3.v7.8.5.min.js');")
