...
- The live trader, Gateway autostart, healthchecks, and exports are all driven by **systemd timers** (timezone-aware, scheduled in **America/New_York**; the Pi clock is **Europe/London**); the **dashboard** is a systemd service **coupled to the trader** (up iff the trader is up — §6/§8).
...
| `monad-dashboard.service` | **with the trader** (no timer) | Read-only FastAPI monitor (`live.dashboard:app`) **coupled to the trader**: `BindsTo=monad-trader.service` (dies when the trader stops/crashes) + `[Install] WantedBy=monad-trader.service` (the trader's `.wants` symlink pulls it up after the trader starts). Tailnet-only at `http://100.76.6.75:8000`. `Restart=on-failure`. Template: `deploy/monad-dashboard.service`. | **enabled** (no timer; started/stopped in lock-step with the trader) |

- **Dashboard ↔ trader coupling:** the dashboard is wired entirely from the dashboard-side unit — `monad-trader.service` is **not** edited. `systemctl enable monad-dashboard` creates `monad-trader.service.wants/monad-dashboard.service`; `BindsTo` provides the stop-side. Net effect: the dashboard runs **iff** the trader runs. To decouple (keep the dashboard always-on like `ctxweb`), drop `BindsTo`/`After=monad-trader.service` and change `WantedBy=` to `multi-user.target`.
...
- **Dashboard:** `live/dashboard.py` (FastAPI, read-only for data; `init_db()` at import is an idempotent no-op). **Now runs as `monad-dashboard.service`, coupled to the trader** (see §6) — bound to the Tailscale IP at `http://100.76.6.75:8000`. Manual/ad-hoc run (localhost): `venv/bin/python -m uvicorn live.dashboard:app --host 127.0.0.1 --port 8000` (from repo root — it imports `src.analysis.run_window`). Endpoints: `GET /`, `GET /health`, `GET /api/ticker/{symbol}`. Install/manage: `sudo cp deploy/monad-dashboard.service /etc/systemd/system/ && sudo systemctl daemon-reload && sudo systemctl enable --now monad-dashboard`.
- **Context-web map (separate service, :8001):** `tools/ctx.py serve` — the context layer's own read-only web view (interactive force-graph, rebuilt fresh each load; no DB/broker/secrets), kept deliberately separate from the fenced trading dashboard. Run ad-hoc: `venv/bin/python tools/ctx.py serve --host 0.0.0.0 --port 8001` → reachable over Tailscale at `http://100.76.6.75:8001`. Persistent: install the unit template `deploy/monad-ctxweb.service` (`sudo cp deploy/monad-ctxweb.service /etc/systemd/system/ && sudo systemctl daemon-reload && sudo systemctl enable --now monad-ctxweb`). Endpoints: `GET /` (the map), `GET /health`. Safe to auto-restart/enable on boot (unlike the trader). Restrict to tailnet-only with `--host 100.76.6.75`.
